...
- **Multi-rep parallel push.** If the original rep is unresponsive but the dealership has multiple Internet Sales reps, after T+48h with no response from the original rep, send the T+48h walk-signal email to the dealership's general Internet Sales mailbox (often `internet@dealer.com` or `sales@dealer.com`) with the original rep CC'd. This sometimes surfaces a more responsive rep at the same store without burning the bridge with the first rep.
