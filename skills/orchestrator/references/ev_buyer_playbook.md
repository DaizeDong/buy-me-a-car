# EV Buyer Playbook

> **last_verified**: 2026-05-18 (skill stress test iteration 5 + P0-P5 consolidation)
> **Data refresh schedule**: state rates / CPO programs / EV incentives / lease parameters should be re-verified annually or upon any user-cited deal that contradicts. The 2026-05-18 timestamp marks last full audit.

Comprehensive reference for new and used EV purchases. Covers federal $7,500 credit (Section 30D), used-EV $4,000 credit (Section 25E), state-by-state EV rebates, charging considerations, range planning, battery health for used EVs, and EV-specific dealer tactics.

## 1. Federal $7,500 New Clean Vehicle Credit (IRC § 30D)

### Vehicle eligibility

| Requirement | Detail |
|---|---|
| Final assembly | North America (US/CA/MX). Verify VIN at fueleconomy.gov tax-credit lookup. |
| MSRP cap | **SUV/truck/van: $80,000**; **car: $55,000** (IRS Notice 2023-16 classifies vehicles; Ioniq 5, Mach-E, ID.4, Model Y = SUV under current guidance) |
| Battery components | $3,750 if % of battery components made/assembled in NA meets year-stepped threshold (60% in 2024, 70% 2026, 80% 2027) |
| Critical minerals | $3,750 if % of critical minerals from US or FTA-partner countries meets threshold (60% in 2024, 70% 2025, 80% 2026) |
| Foreign Entity of Concern (FEOC) | Vehicles with battery components or minerals from China/Russia/Iran/N. Korea sourcing are disqualified (phased in 2024-2025) |

Verify the SPECIFIC VIN's eligibility at fueleconomy.gov — Hyundai/Kia/Ford/GM models flip in and out as battery sourcing changes per production batch.

### Buyer income (Modified AGI) caps

| Filing status | MAGI cap |
|---|---|
| Single | $150,000 |
| Married Filing Jointly | $300,000 |
| Head of Household | $225,000 |

Use the LESSER of current-year and prior-year MAGI. If buyer's MAGI is borderline, advise documenting the lower year carefully (W-2 / 1099 / freelance) BEFORE signing — if they go over in the purchase year, full $7,500 is owed back at filing time (recapture under § 30D(f)(10)).

### POS (Point of Sale) transfer mechanic — the 2024+ option

Since Jan 2024, eligible buyers can transfer the $7,500 credit to a registered dealer at the point of sale, getting the $7,500 as an immediate price reduction instead of waiting for tax filing. Mechanics:

1. **Dealer must be enrolled** in IRS Energy Credits Online (ECO) portal as a "Time of Sale" registered dealer. **NOT ALL FRANCHISE DEALERS REGISTER.** Verify before deposit.
2. Buyer signs Form 8936 (or successor) consenting to transfer at signing; dealer reduces sale price by $7,500.
3. Dealer submits Time of Sale report through ECO portal within 3 days.
4. Buyer receives a copy of the time-of-sale report (KEEP THIS — required at tax filing).
5. Buyer still files Form 8936 with their 1040 to confirm eligibility; if MAGI exceeded, repays the $7,500 there.
6. **Dealer paid by IRS within 72 hours** of submission (this is why dealers love the program — they get paid faster than financing).

### Verification asks for the buyer to make BEFORE deposit

- "Are you enrolled in IRS Energy Credits Online as a Time of Sale registered dealer for the current year?"
- "What is your IRS-issued dealer registration ID number (15-character)?" — get on dealership letterhead.
- "Will my sale paperwork show the $7,500 reduction as a separate line item (mandatory for IRS reporting)?"

If dealer cannot confirm registration: buyer should claim the credit on their own tax return (Form 8936 with 1040 next April) instead of POS transfer. The credit itself is preserved; only time-value is lost (~$300 at 4% opportunity cost on $7,500 over 12 months).

### MSRP cap edge cases

The MSRP cap counts the FACTORY MSRP plus destination/delivery, NOT the dealer-marked-up price and NOT manufacturer-rebated price. If MSRP = $79,500 but dealer adds $2,000 ADM, the vehicle is still eligible (under $80k cap). But if MSRP = $81,000, ADDING any dealer discount does NOT bring it under the cap — the cap is on MSRP, not transaction price.

## 2. Federal $4,000 Used Clean Vehicle Credit (IRC § 25E)

### Vehicle eligibility (used EV)

| Requirement | Detail |
|---|---|
| Sale price | <= $25,000 (the transaction price, not MSRP) |
| Model year | At least 2 years OLDER than current calendar year (e.g., in 2026, must be 2024 MY or earlier) |
| First transfer | Must be the FIRST resale (vehicle that has already been used-EV-credit-claimed is ineligible) |
| Battery capacity | >= 7 kWh |
| Dealer | Must be licensed dealer (not private party) and IRS-registered for POS transfer |

### Buyer income caps (used EV credit)

| Filing status | MAGI cap |
|---|---|
| Single | $75,000 |
| MFJ | $150,000 |
| HoH | $112,500 |

(Half the new-EV caps.)

### Credit amount

**Lesser of $4,000 or 30% of sale price.** So a $14,000 used Bolt = $4,000 (capped). An $8,000 used Leaf = $2,400 (30% of sale).

POS transfer available, same mechanics as new-EV credit.

## 3. State and Local EV Rebates (2026 snapshot — verify each state DOE/clean-energy office for current funding)

| State | Program | Amount | Notes / Provenance |
|---|---|---|---|
| **CA** | Clean Vehicle Rebate Project (CVRP) | **WOUND DOWN Nov 2023**. CVRP closed to new applications. Replaced by CVAP / DCAP (income-targeted, used-EV-only). | Verify cleanvehiclerebate.org status |
| CA | Clean Cars 4 All (low-income retire-and-replace) | Up to $9,500 for low-income trade-up | Air district-specific (Bay Area AQMD, SCAQMD) |
| **NY** | Drive Clean Rebate | **$500-$2,000** depending on EV range | Verify nyserda.ny.gov current funding |
| NJ | Charge Up NJ | **Closed** as of FY2025 — exhausted funding | Verify njdrivegreen.gov |
| **CO** | Innovative Motor Vehicle Credit | **$2,500-$5,000 new BEV** + $2,500 additional for income-qualified | CO state tax credit (must owe CO tax) |
| **IL** | EV Rebate (IL EPA) | **$4,000 new BEV** through 2027, **$1,500 from 2028-2031** | epa.illinois.gov Climate and Equitable Jobs Act |
| **MA** | MOR-EV | **$3,500 new BEV** (sale <= $55k), **$1,500 PHEV**, +$1,500 low-income adder | mor-ev.org |
| **CT** | CHEAPR | **$2,250-$4,250 new BEV** depending on income | ct.gov/deep |
| **OR** | Charge Ahead Rebate | **$2,500 standard, $5,000 low/moderate income** | oregon.gov/deq/aq/programs/Pages/ZEV-Rebate.aspx |
| **TX** | LDPLIP (TCEQ) | **$2,500 historically; ran out of funding 2023, not refunded for FY26**. Verify tceq.texas.gov | No current TX state EV rebate |
| **PA** | Alternative Fuels Incentive Grant | $750 PHEV / **$2,000 BEV** for residents (capped pool) | depweb.state.pa.us |
| **MD** | EVSE Excise Tax Credit | $3,000 BEV (current pool — verify funding) | mva.maryland.gov |
| **WA** | Sales tax exemption | Tax exemption on first $20,000-$25,000 of sale price for BEVs <= $45k | dor.wa.gov |
| **MI** | None at state level (utility-specific only) | -- | -- |
| **FL** | None at state level (utility-specific only) | -- | -- |
| **AZ** | None at state level | -- | -- |
| **OK / TN / KS / KY / SC** | Generally none / utility-specific only | -- | -- |
| **VT** | Drive Electric Vermont | $2,500-$5,000 BEV (income-tiered) | driveelectricvt.com |
| **NH** | None state, $1,000 utility (NHEC) | -- | -- |
| **DE** | Clean Vehicle Rebate | $2,500 BEV | de.gov/dnrec |
| **RI** | DRIVE EV | $2,500 BEV (sale < $60k) | driveev.ri.gov |
| **NM** | Sustainable Building Tax Credit (EV adjacent) | None direct EV state rebate | -- |

**Utility-specific rebates** (everywhere): commonly $500-$1,500 for L2 home charger install. Austin Energy "Plug-In EVerywhere" pays up to $1,200 charger rebate; PG&E EV programs; Dominion; ConEd; etc. Check buyer's utility separately from state rebate.

**Stacking rule:** federal $7,500 + state rebate are STACKABLE in most states (the federal credit is taken first; state then applies to net or gross sale price per state rules). Verify state-specific stacking interaction.

## 4. Charging Considerations

### Home Level 2 Install

| Spec | Typical |
|---|---|
| Voltage / Amperage | 240V / 30-50A (40A most common for full continuous charge speed) |
| NEC continuous derate | 80% — a 40A breaker delivers 32A continuous = 7.7 kW |
| Charge time 20-80% on 7.7 kW | 77.4 kWh Ioniq 5 LR: ~6.5 hrs; 100 kWh F-150 Lightning ER: ~8 hrs |
| Install cost (typical) | $1,000-$2,500 incl. panel upgrade if needed; bare run from sub-panel can be $300-$600 |
| Qmerit, Treehouse, ChargePoint Install | National installer networks; OEM-rebated install partners |
| Common chargers | ChargePoint Home Flex 50A, Wallbox Pulsar Plus 40A, Grizzl-E 40A, Tesla Universal Wall Connector with J1772 (works for non-Tesla) |
| Permitting | Most jurisdictions require permit + inspection for new 240V circuit |
| Utility rebate / incentive | Many utilities pay $300-$1,200 for the install or charger purchase |

### NACS vs CCS1 transition (CRITICAL for 2024-2026 EVs)

NACS = North American Charging Standard (Tesla's plug). All major OEMs announced NACS adoption through 2024-2026:

| OEM | NACS native production start | Pre-NACS adapter availability |
|---|---|---|
| Ford | 2025 MY | Free NACS-to-CCS1 adapter shipped to existing CCS1 owners 2024 |
| GM | 2025 MY | Free adapter program announced 2024 |
| Hyundai/Kia/Genesis | mid-MY 2025 production switch | Hyundai NACS adapter ($300 retail / free through dealer program for existing owners) |
| Rivian | 2025 MY | Adapter program in progress |
| VW Group (VW/Audi/Porsche) | 2025-2026 MY | Adapter program announced |
| Volvo / Polestar | 2025 MY | Adapter shipping |
| Stellantis (Jeep / Ram / Dodge) | 2026 MY | Adapter program announced |
| Tesla | Native NACS always | n/a |

**Ask the dealer about the SPECIFIC VIN:** is this build CCS1 or native NACS? If CCS1, is the adapter included in delivery or available through manufacturer program? Without the adapter, the vehicle cannot DC fast charge at Tesla Superchargers — which is ~50% of high-power DC infrastructure in many regions.

### Public DC Fast Charging Networks

| Network | Notes |
|---|---|
| Tesla Supercharger | Largest network. NACS-native. Non-Tesla access requires NACS port or adapter. ~$0.43-$0.55/kWh typical. |
| Electrify America | 350kW max, CCS1 + NACS rolling out. ~$0.43-$0.56/kWh. Strong I-10/I-95 corridor coverage. Free 2-3yr included with many new EV purchases (Ioniq 5: 2 years free 30-min sessions). |
| EVgo | Many 350kW sites + 50kW legacy. CCS1, NACS adapters in deployment. ~$0.30-$0.55/kWh + membership tiers. |
| ChargePoint | DC fast at limited sites; mostly L2 destination charging. |
| Francis Energy / Blink / Volta | Smaller regional networks. |

Route-plan via ABRP (A Better Route Planner) or PlugShare. For a 250-300mi round trip with one charge stop, plan for 15-20 min on 350kW or 35-50 min on 150kW.

### 800V vs 400V architecture

- **800V native (E-GMP platform):** Ioniq 5/6, EV6, GV60, Genesis GV70 EV. 10→80% in 18 min on 350kW. Future-proof for ultra-fast charging.
- **400V (most other EVs):** Tesla, Ford Mach-E, GM Ultium, VW MEB, Nissan Ariya. 10→80% in 25-40 min on 150-250kW.
- **Relevance to negotiation:** 800V is a real differentiator but the buyer's actual experience depends on whether they have access to 350kW chargers (need DC infrastructure that supports it — most highway corridors do).

## 5. Range Planning

### EPA-rated vs real-world range

EPA range is measured at 65mph mixed cycle, 70F ambient. Real-world deltas:

| Condition | Range vs EPA |
|---|---|
| 65mph highway, mild temp | 100% (matches) |
| 75mph highway | -15-20% |
| 80mph highway | -25-30% |
| 0F cold weather | -30-40% (battery warms slowly + cabin heat) |
| 95F + AC | -10-15% |
| Towing 4,000lb | -50% typical (varies by EV) |
| Roof rack / bike rack | -10-15% |

### Range minimum sizing rule

(One-way commute × 2 + max errand loop + buffer) >= overnight-charge-replenishable range.

Examples:
- 40mi round-trip commute + 20mi errands + 25% buffer = 75mi/day → any modern EV (200mi+ EPA) handles with 4-5 day cushion
- 80mi round-trip + 30mi extra + buffer = 138mi/day → needs ~180mi real-world range (220mi EPA-rated)
- Frequent 165mi one-way road trips → either L2-only EV with mid-trip DC fast charge (15-20min) or 350mi-range EV with overnight at destination

### AWD vs RWD range trade-off

Most dual-motor EVs lose 35-50mi of range vs single-motor (RWD) of the same trim. The trade-off math:

- For buyers in mild-climate states (CA, TX, FL): AWD is a luxury, RWD is fine
- For buyers in snow states (NJ, NY, PA, CT, MA, ME, MI, CO, MN): AWD is a real winter safety value; range loss is acceptable
- Mechanical engineers, performance-oriented buyers: AWD often preferred for 0-60 / on-ramp acceleration

## 6. Battery Health for Used EVs (State of Health, SoH)

For any used EV transaction, request the **battery State of Health report** before close. Methods:

1. **OEM diagnostic readout** — dealer service department can pull on Hyundai/Kia/Ford/GM/Tesla service tools
2. **Recurrent report** (Recurrent Auto) — third-party SoH service, $20-50, accepts VIN
3. **EV Auto / Smart Charging Group** — independent EV-specialty shops

### SoH thresholds for negotiation

| SoH | Verdict (sub-3yr car) | Action |
|---|---|---|
| >= 95% | Excellent | Normal price |
| 90-95% | Healthy | Normal price; document for resale |
| 85-90% | Below average for age | Negotiate -$1,000-$2,000 |
| 80-85% | Concerning | Negotiate -$2,000-$4,000 OR walk |
| < 80% | Battery aging risk | Walk OR confirm battery warranty coverage |

### Battery warranty standard

**Federal minimum:** 8 yrs / 100,000 mi battery warranty on all new EVs. Most OEMs cover capacity loss below 70% within warranty period.

- Hyundai/Kia: 10 yr / 100k mi (industry-leading; lifetime to original owner on some MY)
- Tesla: 8 yr / 100k-150k mi (varies by model)
- Ford / GM / VW: 8 yr / 100k mi (federal minimum)
- Toyota: 10 yr / 150k mi on bZ4X (longest)

**Warranty transferability:** verify on used purchases. Most OEM battery warranties transfer with vehicle.

## 7. Depreciation Patterns

EVs have historically depreciated FASTER than comparable ICE vehicles, especially:

- 2017-2020 Nissan LEAF: severe depreciation (battery degradation rep)
- 2018-2022 Tesla Model 3 / Y: held value well but softening 2024-2026 as supply increased
- Bolt EV (2017-2023): severe post-recall depreciation
- Ford Mach-E, F-150 Lightning: 2022-2024 high depreciation as MSRPs rose then fell
- Hyundai/Kia E-GMP (Ioniq 5, EV6): moderate depreciation, better than legacy EVs

**Buyer guidance:** assume 50-60% 5-year retained value on new EVs vs 60-70% for comparable ICE. Use this in trade-in estimate planning if buyer expects to trade in 3-5 years. For cash buyers planning to keep 8-10 years, this is less load-bearing (matching battery warranty length).

Used EV buying: this is a BUYER'S market in 2026 — used Bolts, Leafs, and older Mach-E inventory pricing is well below replacement cost. Federal $4,000 used credit on <$25k vehicles makes used EVs very competitive.

## 8. Dealer Tactics Unique to EVs

### EV Prep Fee / Battery Conditioning Fee / Charge Cable Fee / EV Delivery Setup Fee

These line names ($500-$1,500) are **ADM-equivalent** — Hyundai/Kia/Ford/GM PDI (Pre-Delivery Inspection) already covers battery conditioning, charge cable delivery, and setup as factory-reimbursed items through holdback. Adding a separate "EV Prep" line is double-billing.

**Add to ADM kill list (gotcha D9 + outreach_strategy.md New-Car ADM Detection section):**
- EV Prep Fee
- Battery Conditioning Fee
- Charge Cable Fee (the L1 OEM charge cable is a factory-included accessory)
- EV Delivery Setup Fee
- High-Voltage Inspection Fee (a few dealers tried this in 2024)

Demand removal in first counter per D9 single-round, no-coupling rule.

### Lease-pricing vs cash-pricing gap

Because of the **Section 45W commercial vehicle credit** ($7,500 with no MSRP/AGI cap), OEMs (especially Hyundai, Ford, Toyota) pass this through aggressively to **lessees** via HMF / FMC / TFS captive lease deals. Cash buyers see worse pricing than lessees on the same VIN. The gap is real and structural.

**Buyer guidance:** if budget-flexible and lease-acceptable, run the lease numbers — sometimes a 24-36mo lease at full $7,500 captive subsidy then a lease-buyout produces lower effective cost than cash purchase. This is a financing buyer-type analysis (cross-ref payment_methods.md). For firm-cash buyers (like the TX EV example), accept the gap as the cost of avoiding finance.

### IRS registration as a leverage point

If a dealer is NOT IRS-registered, this is a real negotiation lever — buyer is losing 12 months of time value on $7,500 (~$300 at 4%). Use as $300-$500 of additional discount leverage OR route to next dealer.

### "EV is in transit" stall

Some dealers will quote tight pricing on a vehicle that doesn't physically exist on the lot ("in transit", "factory order"). Confirm:
- VIN (vehicles in transit have assigned VINs)
- Expected arrival date in writing
- Refundable deposit (NOT non-refundable)
- Price lock (in writing — protect against re-quote on arrival)

## 9. EV-Specific Phase 6 Checklist

Before sending a counter on an EV:

- [ ] Dealer IRS Energy Credits Online registration verified (letterhead or portal lookup)
- [ ] VIN-specific fueleconomy.gov eligibility for full $7,500 confirmed
- [ ] Buyer MAGI under cap (Single $150k / MFJ $300k) — buyer-confirmed
- [ ] No EV Prep / Battery Conditioning / Charge Cable / EV Delivery line items (or demanded removed)
- [ ] NACS vs CCS1 port type confirmed for the VIN
- [ ] NACS adapter included / available through manufacturer program (if CCS1)
- [ ] Hyundai Customer Cash / Ford BlueOval / GM Customer Cash / etc. current incentive netted into sale price (not pocketed by dealer)
- [ ] State EV rebate (if any) confirmed not double-claimed by dealer
- [ ] Home L2 charging confirmed (or buyer aware of public-only charging cost premium)
- [ ] Battery warranty registered to buyer at delivery (new) OR SoH report obtained (used)

## 10. EV-Specific Walk Conditions

- Dealer not IRS-registered AND won't waive equivalent ~$300 time-value → walk to registered dealer
- "EV Prep Fee" or similar refused to be removed (not reduced) → walk per D9
- VIN's eligibility for $7,500 federal cannot be confirmed via fueleconomy.gov → walk
- Battery warranty already started ticking before delivery (loaner / demo with significant miles) → either renegotiate or walk
- For used EVs: SoH report refused → walk
- "Lease only" pricing structure (some Hyundai dealers refuse to sell EVs cash, push lease) → route to dealer that will sell cash
